...
all test profiles; isolated E2E is the only environment allowed to include them.
Result-set snapshots expire after 30 days.
...
test profile，只有隔離 E2E 環境可明確納入；result set 30 天後到期。

...
assert.equal(second.result_set_id, first.result_set_id); assert.equal(second.matches.length, 1); assert.equal(second.page, 2);
...
const report = { passed: true, runId: artifact.runId, siteUrl: site, assertions: { firstPublishWithoutDisplayName: true, publicSlugLength: 22, publishIdempotentReplay: true, deprecatedDisplayNameRejected: true, profiles: 12, eligibleCandidates: 11, page1: 10, page2: 1, deterministicTopTwo: true, unchangedGraphReusesSet: true, changedGraphCreatesSet: true, inviteIdempotent: true, previewReadOnly: true, rejectFinal: true, connectionRecords: 2, connectionEmails: 2, unauthorizedConnectionRejected: true, privacyPlaceholderStable: true, connectedSnapshotRetainedWhenPrivate: true, privateSocialImageRevoked: true, missingSocialImageRevisionRejected: true, staleSocialImageRevisionRejected: true, socialImage: "1200x630", socialQrDecodedAt: ["1200x630", "600x315"] } };

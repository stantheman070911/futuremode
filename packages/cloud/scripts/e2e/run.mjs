...
const image = await fetch(`${site}/og/profile/${profileC.public_slug}.png?version=${profileC.version_id}`); assert.equal(image.status, 200); assert.match(image.headers.get("content-type") || "", /image\/png/); const imageBytes = Buffer.from(await image.arrayBuffer()); assert.equal(imageBytes.readUInt32BE(16), 1200); assert.equal(imageBytes.readUInt32BE(20), 630);

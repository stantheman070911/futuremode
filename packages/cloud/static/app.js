...
const COPY = {
  en: {
    ...
    "match.back": "Back to matches", "match.loading": "Opening match reason", "match.ownerPitch": "Owner pitch", "match.viewDetail": "See details", "match.score": "Match score {score} / 100", "match.q1": "01 · What we both care about", "match.q2": "02 · Same reason, right now", "match.q3": "03 · What we could discuss today", "match.evidence": "Evidence · {label}", "match.connected": "You are connected", "match.start": "Start with this", "match.waiting": "Waiting for Ren H.", "match.simulate": "Demo · Simulate Ren accepting", "match.sent": "Invitation sent. Contact appears only after mutual acceptance.", "match.passed": "Passed. This decision is recorded and cannot be undone; they are not told.", "match.passConfirmTitle": "Pass on {name}?", "match.passConfirmBody": "This cannot be undone. They are not told.", "match.confirmPass": "Confirm pass", "match.notNow": "Not now", "match.accept": "Accept", "match.invite": "Send invitation", "match.unavailable": "Unavailable", "match.unavailableBody": "Match unavailable", "match.demoData": "Test data · not a real person", "match.testCode": "Test ID {code}", "match.fixtureOnly": "This test profile is visible only to you and cannot receive an invitation.", "match.demoAcceptance": "Demo · simulated acceptance", "match.demoSent": "Demo invitation sent", "match.demoAccepted": "Demo · Ren H. simulated acceptance", "match.passNotice": "Passed. They are not told.",
  ...
  "zh-Hant": {
    ...
    "match.back": "返回配對", "match.loading": "正在開啟配對理由", "match.ownerPitch": "Owner 介紹", "match.viewDetail": "查看詳細介紹", "match.score": "配對分數 {score} / 100", "match.q1": "01 · 我們都關心什麼", "match.q2": "02 · 此刻出於相同理由", "match.q3": "03 · 今天可以聊什麼", "match.evidence": "依據 · {label}", "match.connected": "你們已連結", "match.start": "可以這樣開場", "match.waiting": "等待 Ren H. 回覆", "match.simulate": "Demo · 模擬 Ren 接受", "match.sent": "邀請已送出。只有雙方接受後才會顯示聯絡方式。", "match.passed": "已略過。此決定已記錄且無法復原；對方不會收到通知。", "match.passConfirmTitle": "略過 {name}？", "match.passConfirmBody": "此決定無法復原，對方不會收到通知。", "match.confirmPass": "確認略過", "match.notNow": "現在不要", "match.accept": "接受", "match.invite": "寄送邀請", "match.unavailable": "無法使用", "match.unavailableBody": "配對目前無法使用", "match.demoData": "測試資料•非真實人", "match.testCode": "測試編號 {code}", "match.fixtureOnly": "這是只提供給你的測試 profile，不會寄出邀請。", "match.demoAcceptance": "Demo · 模擬接受", "match.demoSent": "Demo 邀請已送出", "match.demoAccepted": "Demo · Ren H. 已模擬接受", "match.passNotice": "已略過，對方不會收到通知。",
...
function testDataMarker(peer, forced = false) {
...
function connectionScreen(connectionId) {
  ...
  return shell(`<a href="/invitations" data-link class="eyebrow" style="text-decoration:none">← 返回邀請</a><h1 class="page-title">${esc(t("connection.title"))}</h1>${testDataMarker(peer)}${profilePortrait(peer, "detail", "hero-portrait")}<p class="page-intro">你和 ${esc(peerPresentationName(peer))} 都明確接受了這次介紹。</p>${publicProfileDocument(peer.profile)}<div class="notice success"><span class="field-label">${esc(t("connection.email"))}</span><p><a href="mailto:${esc(peer.contact_email)}">${esc(peer.contact_email)}</a></p></div><section class="connection-ai"><p class="eyebrow">${esc(t("connection.aiEyebrow"))}</p><h2>${esc(t("connection.aiTitle"))}</h2><p>${esc(t("connection.aiBody"))}</p><div class="button-stack"><button class="button primary" data-action="write-connection-email" data-ai="ChatGPT">${esc(t("connection.chatgpt"))}</button><button class="button" data-action="write-connection-email" data-ai="Claude">${esc(t("connection.claude"))}</button></div></section>${manualPrompt}`, { nav: true, active: "invitations" });
}
...
function matchDetailScreen(matchId) {
  ...
  const demoMarker = testDataMarker(match.peer, runtime.demo);

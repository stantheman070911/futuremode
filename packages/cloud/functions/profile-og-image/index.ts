...
import { profileImageRevision, readyProfileImage } from "../shared/profile-image.js";
...
export async function portraitPngDataUri(bytes: Uint8Array): Promise<string> {
  const png = await sharp(Buffer.from(bytes)).png({ compressionLevel: 9 }).toBuffer();
...
export async function handler(event: APIGatewayProxyEventV2): Promise<APIGatewayProxyResultV2> {
  try {
    ...
    if (raw === "site") svg = renderSocialSvg({ fontBase64: Buffer.from(fontBuffer()).toString("base64") });
      ...
      const requestedImageRevision = event.queryStringParameters?.image;
      const image = readyProfileImage(current);
      const currentImageRevision = profileImageRevision(current);
      if (!versionId || versionId !== current.versionId || !requestedImageRevision || requestedImageRevision !== currentImageRevision || !image?.thumbnailKey) {
      ...
      if (!object.Body) return plain(503, "service unavailable");
      portraitDataUri = await portraitPngDataUri(await object.Body.transformToByteArray());
      svg = renderSocialSvg({
        profile: {
    ...
    return { statusCode: 200, headers: { "content-type": "image/png", "cache-control": raw === "site" ? "public, max-age=300, s-maxage=300" : "public, max-age=0, must-revalidate", "x-content-type-options": "nosniff" }, isBase64Encoded: true, body: Buffer.from(png).toString("base64") };
  } catch (error) {
